...
export class GameComponent {
	...
	public handleSelection( event: GridSelectionEvent ) : void {
		...
		let selectedLetters = event.letters.join( "" ).toLowerCase();
		let selectedLettersInverse = event.letters.reverse().join( "" ).toLowerCase(); // CAUTION: In-place reverse.
	...
	private getGame() : Game {
		...
		let selectedConfig = this.getRandom( configurations );
	...
	private getRandom<T>( collection: T[] ) : T {
		...
		let randomIndex = _.random( collection.length - 1 );
