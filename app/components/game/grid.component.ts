...
@Component({
	...
	templateUrl: "../../templates/grid.component.htm"
})
export class GridComponent {
	...
	public endSelection() : void {
		...
		let isSubsumed = this.selections.some(
		...
		if ( ! isSubsumed ) {
			...
			let selectedLetters = this.pendingSelection.map<string>(
	...
	public isSelected( row: number, column: number ) : boolean {
		...
		let result = this.selections.some(
			( selection: GridSelection ) : boolean => {
...
export class 	GridSelection {
	...
	public includes( location: GridLocation ) : boolean {
		...
		let isFound = this.gatherLocations().some(
	...
	public isSubsumedBy( selection: GridSelection ) : boolean {
		...
		let isConflict = this.gatherLocations().every(
	...
	public map<T>( callback: GridSelectionMapFunction ) : T[] {
		...
		let result = this.gatherLocations().map(
	...
	public update( newTo: GridLocation ) : void {
		...
		let deltaRow = Math.abs( newTo.row - this.from.row );
		let deltaColumn = Math.abs( newTo.column - this.from.column );
		let maxDelta = Math.max( deltaRow, deltaColumn );
	...
	private gatherLocations() : GridLocation[] {
		...
		let rowIncrement = this.getIncrement( this.from.row, this.to.row );
		let columnIncrement = this.getIncrement( this.from.column, this.to.column );
		let iRow = this.from.row;
		let iColumn = this.from.column;
